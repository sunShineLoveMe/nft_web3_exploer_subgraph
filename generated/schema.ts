...
export class QLTransfer extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save QLTransfer entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type QLTransfer must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("QLTransfer", id.toString(), this);
    }
  }

  static load(id: string): QLTransfer | null {
    return changetype<QLTransfer | null>(store.get("QLTransfer", id));
  }
